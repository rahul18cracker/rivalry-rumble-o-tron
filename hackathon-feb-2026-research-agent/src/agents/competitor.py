...
async def run_competitor_agent(task: str, companies: list[str] | None = None) -> dict[str, Any]:
    ...
    """
    logger.info("competitor_agent.run.start", task=task, companies=companies)
    try:
        ...
        logger.info("competitor_agent.run.end", companies=companies, tool_call_count=len(tool_calls))
        return {
            "task": task,
            "companies": companies,
        ...
        logger.error("competitor_agent.run.error", task=task, error=str(e))
        return {
            "error": str(e),
            "response": "",
            "task": task,
